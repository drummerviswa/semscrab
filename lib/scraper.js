...
export async function runSEMSScraper(rollNumber) {
  ...
  try {
    ...
    console.log(`Found ${options.length} semester options to scrape.`);
    ...
    for (const option of options) {
      ...
      // Click Go/Submit if it exists, otherwise wait
      ...
      if (submitBtn) {
        ...
        await page.waitForTimeout(2000); // Fallback wait for AJAX
      }
      ...
      if (!semesterNo) {
        console.log(`Could not determine semester number for session "${option.text}". Skipping...`);
        continue;
      }
