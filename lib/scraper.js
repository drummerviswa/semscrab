...
export async function runSEMSScraper(rollNumber) {
  ...
  try {
    ...
    page.on('request', request => {
      if (request.url().includes('login/student') && request.method() === 'POST') {
        console.log('--- DIAGNOSTIC POST LOGIN REQUEST ---');
    ...
    for (let i = 0; i < sortedOptions.length; i++) {
      ...
      let semesterNo = await page.evaluate(() => {
      ...
      for (const row of scrapResults.dataRows) {
        ...
        const isNotPublished = cellColor === 'rgb(89,70,67)' || cellColor === '#594643' || !grade || grade.trim() === '';
        
        let finalGrade = grade ? grade.trim().toUpperCase() : '';
        let gradePoints = 0;

        if (isNotPublished || finalGrade === '' || finalGrade === '-' || finalGrade === 'NP') {
          status = 'NOT_PUBLISHED';
          finalGrade = 'NP';
          gradePoints = 0;
        } else if (finalGrade === 'W') {
          status = 'W';
          finalGrade = 'W';
          gradePoints = 0;
        } else {
          gradePoints = getGradePoints(finalGrade);
          if (finalGrade === 'U' || finalGrade === 'RA' || finalGrade === 'SA') {
            status = 'FAIL';
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Accumulate points, GPA credits, and earned credits only for published passed courses
      scrapedData.forEach(g => {
        if (g.status !== 'NOT_PUBLISHED') {
          // Exclude failed/arrear papers (whose gradePoints is 0) from the GPA calculation
...
export async function parseManualHTML(rollNumber, semesterNo, htmlString) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const userExists = await tx.user.findUnique({ where: { rollNumber } });
      if (!userExists) {
      ...
      semesterGrades.forEach(g => {
        // Exclude failed/arrear papers from GPA calculation (gradePoints = 0 or status/grade indicates failure)
        const isPassed = g.status === 'PASS' || g.gradePoints > 0;
        if (isPassed) {
